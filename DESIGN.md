...
- **Night Azure** (`dark-azure`, #3e9bff): Carries large primary actions and promotion color fields on deep-sea surfaces.

### Secondary

- **Daylight Sun** (`light-sun`, #ffd51e): Marks compact labels, selection, and small moments of orientation in light mode.
- **Signal Coral** (`dark-coral`, #ff725b): Owns dark-mode links, icons, numbered micro-badges, accordion controls, active borders, focus, selection, and urgent validation.
...
**The Two-Sky Rule.** Light mode stays cobalt-and-sun. Dark mode keeps Azure for large action and product fields while Coral owns smaller interactive text, icons, and active borders. Never invert one palette mechanically into the other.

**The Route-First Rule.** Use vivid color to identify a workflow, action, or state; do not scatter it as decoration.

**The Shared-Hero Rule.** B2B product heroes use the current canvas rather than a full ochre or coral wash. Their identity stays in compact labels, workflow badges, and other deliberate signals so the page remains part of the same site.
...
- **Secondary / Quiet:** Paper-colored surface, current ink, and a one-pixel rule; hover moves to the interactive accent surface and interactive accent border.
...
The proof strip is a low-height evidence row rather than a metric boast. Four equal cells use muted 0.72rem labels, one-pixel divisions, and no shadow; the first fact takes the interactive accent—Cobalt in light mode, Coral in dark mode. It becomes two columns and then one on small screens.

### Lists

Lists use crisp Lucide Check or X vectors rather than text glyphs or dots. The icon is 16px, aligns with the first text line, and carries meaning through shape as well as color. Cobalt carries these markers in light mode and Coral in dark mode. Divider lists never draw a rule below their final item. FAQ accordions use a one-pixel accent rule and an accent-colored `+ / −` control.
